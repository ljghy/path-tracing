...
Camera::Camera(const glm::vec3 &_pos, const glm::vec3 &_dir, const glm::vec3 &_up, float _hfov, float _ratio)
    : camPos(_pos), camDir(glm::normalize(_dir)), camUp(glm::normalize(_up)), hFov(glm::radians(_hfov)), ratio(_ratio)
{
}
...
void Camera::generateRay(Ray &r, float u, float v)
{
    ...
    r = Ray(camPos, 0.5f / tanf(0.5f * hFov) * camDir + (u - 0.5f) * uAxis + (v - 0.5f) / ratio * vAxis);
}